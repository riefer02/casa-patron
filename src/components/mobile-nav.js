...
export default function MobileMenu({ toggle, navLinks, setMobileNavActive }) {
  ...
  return (
    <Fade toggle={toggle}>
      <div className="relative z-30 sm:hidden h-[94vh]" id="mobile-menu">
        <div className="p-10 h-full flex items-center align-center flex-col">
          {navLinks.map((navLink, index) => (
            <Link
              key={index}
              to={navLink.url}
              className="mb-3 px-3 py-5 text-2xl  shadow-lg bg-gray-100 rounded-[72.5px] min-w-[100%] text-center"
              aria-current="page"
